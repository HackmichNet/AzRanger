                ...
                foreach (DirectoryRoleAssignment assignment in role.pimRoleAssignmentsEligible)
                {
                    ...
                    else
                    {
                        ...
                        if (assignment.directoryScope.odatatype.Equals("#microsoft.graph.application"))
                        {
                            foreach (AzurePrincipal p in principalsToAssign)
                            {
                                role.AddEligibleMemberScoped(new Tuple<AzurePrincipal, AzurePrincipal>(p, new AzurePrincipal(scopeId, AzurePrincipalType.Application)));
                            }
                            continue;
                        }
                        if (assignment.directoryScope.odatatype.Equals("#microsoft.graph.servicePrincipal"))
                        {
                            if (tenant.ServicePrincipals[scopeId].appOwnerOrganizationId == tenant.TenantId)
                            {
                                foreach (AzurePrincipal p in principalsToAssign)
                                {
                                    role.AddEligibleMemberScoped(new Tuple<AzurePrincipal, AzurePrincipal>(p, new AzurePrincipal(scopeId, AzurePrincipalType.ServicePrincipal)));
                                }
                            }
                            continue;
                        }
